...
public class Vertex extends Element {
	...
	public Vertex(int id, Graph graph, byte label) {
	...
	public ArrayList<Integer> getEdges(Direction direction, Byte ...edgeLabels)
			throws IOException {

		List<Byte> labelList = Arrays.asList(edgeLabels);
	 ...
	 * @throws IOException
	 */
	public ArrayList<Integer> getEdges(Direction direction) throws IOException {
		// dbPath = path;
	...
	public ArrayList<Integer> getNeighborEdges(ArrayList<Map<Byte,Short>> neighborCountList, int offset, int totalNeighbors, Direction direction) {
		...
		try {
			...
			for (int i = offset / Constants.MAX_EDGES_NODES_DAT; i < Math.ceil(maxFiles); i++) {
				...
				dataFile = new RandomAccessFile(graph.dbPath + "relfile" + i
						+ ".dat", "r");
	...
	public ArrayList<Integer> getNeighborEdges( ArrayList<Integer> offsetList , ArrayList<Map<Byte,Short>> neighborCountList, ArrayList<Byte> labelList, Direction direction) {
		...
		try {
			...
			for(int offset=0; offset<offsetList.size(); offset +=2){
				...
				byte edgeLabel = labelList.get((offset/2)%labelList.size());
	...
	public Map<String,Object> getAttributes(String... keys){
		return this.graph.mysql.getAttributes("vertex", this.id, keys);
	}
